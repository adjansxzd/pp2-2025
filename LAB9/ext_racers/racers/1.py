...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        self.base_speed = 5 # Начальная скорость
        self.speed = self.base_speed
        self.max_speed = 10 # Максимальная скорость
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(image_coin, (20 + self.weight*5, 20 + self.weight*5))
        self.rect = self.image.get_rect()
        self.generate_random_position()

    def generate_random_position(self):
        self.weight = random.choice([1, 2, 3])  # Обновляем вес при генерации
        self.image = pygame.transform.scale(image_coin, (20 + self.weight*5, 20 + self.weight*5))
